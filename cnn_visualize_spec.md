...
- 内容: テストセットからランダムに 16 枚を選択し 2行×8列で表示
  - 正解: 緑の枠線 / 不正解: 赤の枠線
...
- CIFAR-10 データは初回実行時に自動ダウンロードされ `data/` に保存される。
- `save_test_predictions` の画像選択は `np.random.choice` による毎回ランダムなため、実行のたびに結果が異なる。
